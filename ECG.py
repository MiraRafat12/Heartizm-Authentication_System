...
class ECG:
    ...
    def feature_exctraction(self):
        ...
        PR_amplitudes = self.Amplitudes(signal, pr_peaks, p_peaks)
...
class sql_ecg():
    ...
    def insert_model(self, model):
        ...
        db = sqlite3.connect('Heartizm Sam.db')
        cursor = db.cursor()
    ...
    def fetch(self, cols, table):
        connection = sqlite3.connect('Heartizm Sam.db')
...
def authentication_train():
    ...
    print(other_users_features)
    other_users_features.to_csv('final data.csv')
    X = df.iloc[:, :-1]
    y = df.iloc[:, -1]
